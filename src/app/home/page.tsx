        ...
        <div className={styles.generals}>
            <div className={styles["stat-box"]}>
                ...
                <div className={styles["stat-box-info"]}>
                    <Attribute className={styles["stat-attribute"]} title={"Boss name"}
                               text={nextBossToKill?.name || ""}/>
                    <Attribute className={styles["stat-attribute"]} title={"Boss location"}
                               text={`${nextBossToKill?.region}, ${nextBossToKill?.location}`}/>
                    <Attribute className={styles["stat-attribute"]} title={"Rewards"}
                               text={nextBossToKill && nextBossToKill.drops.length > 0 ? arrayToString(nextBossToKill?.drops!) : "Nothing"}/>
                </div>
            ...
            <div className={styles["stat-box"]}>
                <h1>General Stats</h1>
                <div className={styles["stat-box-info"]}>
                    <Attribute className={styles["stat-attribute"]} title={"Total bosses defeated"}
                               text={`${totalDefeatedBosses} / ${totalBosses}`}/>
                    <Attribute className={styles["stat-attribute"]} title={"Total deaths"} text={`${totalDeaths}`}/>
                    <Attribute className={styles["stat-attribute"]} title={"Most tried boss"}
                    ...
                    regionDeaths.map(([region, regionData]) => {
                        if (regionData.deaths > 0) {
                            return <Accordion key={region} title={region}>
                                <Attribute className={styles["stat-attribute"]} title={"Total Deaths"}
                                           text={`${regionData.deaths}`}/>
                                <Attribute className={styles["stat-attribute"]} title={"Most tried boss"}
                                           text={`${regionData.mostTried?.name}, ${regionData.mostTried?.tries}`}/>
                            </Accordion>
                        }
                    }) :
